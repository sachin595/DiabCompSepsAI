...
def download_model():
    try:
        if not os.path.exists('diabetes.joblib'):
            url = 'https://drive.google.com/uc?id=1rcMwphsHTsDVGd-kyilwvJ7pWP98jvNw'
...
def predict(inputs):
    ...
    inputs['drenainsf'] = drenainsf_map[inputs['drenainsf']]
...
def decode_predictions(y_pred):
    ...
    decoded_predictions = {
        "Presence of sepsis": sepis_mapping[y_pred[1]],
        "Presence of wound infection": wound_infection_mapping[y_pred[0]],
    }
...
st.title(' DiabCompSepsAI: Prediction of Postoperative Complications in Diabetic Patients ')
...
with st.form(key='prediction_form'):
    ...
    drenainsf = st.selectbox('Deep renal insufficiency', options=["No", "Yes"])
